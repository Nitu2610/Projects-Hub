...
import type { Address } from "../../redux/api/addressApi";
...
type AddressFormErrors = Partial<
  Record<keyof Omit<Address, "_id">, string>
>;
...
export const AddressForm = ({
  ...
  const getBackendError = (field: string) => {
    return backendErrors.find(
      error => error.path === field
    )?.msg;
  };

  const handleChange = (
    field: keyof Omit<Address, "_id">,
    value: string
  ) => {
    setFormData(prev => ({
      ...prev,
  ...
  const fullNameError =
    errors.fullName || getBackendError("fullName");

  const phoneError =
    errors.phone || getBackendError("phone");

  const addressLine1Error =
    errors.addressLine1 ||
    getBackendError("addressLine1");

  const cityError =
    errors.city || getBackendError("city");

  const stateError =
    errors.state || getBackendError("state");

  const postalCodeError =
    errors.postalCode ||
    getBackendError("postalCode");

  const countryError =
    errors.country ||
    getBackendError("country");
